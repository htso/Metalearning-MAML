...
Usage Instructions:
    Product of Sine:
        ...
        python main_ht.py --datasource=fun --logdir=logs/fun --metatrain_iterations=100000 --norm=None --update_batch_size=100 --meta_batch_size=10 --train=False

    10-shot sinusoid:
        python main_ht.py --datasource=sinusoid --logdir=logs/sine --metatrain_iterations=70000 --norm=None --update_batch_size=10

    Use trained model for testing:    
        python main_ht.py --datasource=sinusoid --logdir=logs/sine --metatrain_iterations=70000 --norm=None --update_batch_size=10 --train=False

    10-shot sinusoid baselines:
        python main_ht.py --datasource=sinusoid --logdir=logs/sine --pretrain_iterations=70000 --metatrain_iterations=0 --norm=None --update_batch_size=10 --baseline=oracle
        python main_ht.py --datasource=sinusoid --logdir=logs/sine --pretrain_iterations=70000 --metatrain_iterations=0 --norm=None --update_batch_size=10
    ...
    20-way, 1-shot omniglot:
        python main_ht.py --datasource=omniglot --metatrain_iterations=40000 --meta_batch_size=16 --update_batch_size=1 --num_classes=20 --update_lr=0.1 --num_updates=5 --logdir=logs/omniglot20way/
...
========== EXPLAINING EVERYTHING TO MYSELF (HT) =======================================================================================
    ...
    The first K data points are the inputa, while the rest is assigned to inputb, where K is 'update_batch_size'. These two variables
    have shape,
          
            inputa, inputb = (batch size, 2*K, 1) 

    where batch size is the meta_batch_size, ie. the number of meta datasets. For the sinusoid generator, 2*K is the number of points 
    ...
    for itr in range(resume_itr, FLAGS.pretrain_iterations + FLAGS.metatrain_iterations):
        ...
        if 'generate' in dir(data_generator):
            ...
            if FLAGS.datasource == 'fun':
                batch_x, batch_y, amp, phase = data_generator.generate(randomize=True, train=False, x_range=[0.0,3.0], add_noise=True, noise_sd=0.1)
            ...
            # Splitting a batch into two halves. inputa is the first half, inputb is the rest. 
            # Note : inputa and inputb come from the same function; they are just different pairs of {x_i, f(x_i)}
            # so, the submodels M({x}_i, theta_i) are learning and testing on data from the same function.
            inputa = batch_x[:, :num_classes*FLAGS.update_batch_size, :] # a is for few-shot training
            labela = batch_y[:, :num_classes*FLAGS.update_batch_size, :]
            inputb = batch_x[:, num_classes*FLAGS.update_batch_size:, :] # b is for validation
        ...
        # =============================================
        #
        # Q : what does sess.run(metatrain_op) return ?? Not clear why these elements are returned.
        ...
        if (itr!=0) and (itr % TEST_PRINT_INTERVAL) == 0 and FLAGS.datasource !='sinusoid' and FLAGS.datasource !='fun':
        ...
        elif (itr!=0) and (itr % TEST_PRINT_INTERVAL) == 0 and FLAGS.datasource =='fun':
            batch_x, batch_y, amp, phase = data_generator.generate(num_pts=1000, randomize=True, batch_size=5, x_range=[0.0,10.0], train=False)
            inputa = batch_x[:, :500, :]
            inputb = batch_x[:, 500:, :]
            labela = batch_y[:, :500, :]
            labelb = batch_y[:, 500:, :]
            ...
            input_tensors = [model.total_loss1, model.total_losses2[FLAGS.num_updates-1]]
            result = sess.run(input_tensors, feed_dict)
            print('[fun] Validation total_loss1: ' + str(result[0]) + ', total_losses2[-1] : ' + str(result[1]))
...
def main():
    if FLAGS.datasource == 'sinusoid':
        if FLAGS.train:
            ...
            test_num_updates = 5
    # [HT] More complicated functional shapes, e.g. product of sine/cosine
    elif FLAGS.datasource == 'fun':
        if FLAGS.train:
            test_num_updates = 5
        else:
    ...
    if FLAGS.datasource == 'sinusoid':
        ...
        data_generator = FunGenerator(num_pts=FLAGS.update_batch_size*2, batch_size=FLAGS.meta_batch_size, randomize=False)
    else:
