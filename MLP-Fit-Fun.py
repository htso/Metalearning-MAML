"""
MLP-Fit-Fun.py -- fit two-layer fc MLP on some functional shape. The shapes take the form of 
      product of sinusoidal functions. For example,
      
           f(x) = sin(w*x + phase) * cos(w*x + phase)
...
from FunGenerator_old import FunGenerator
...
def run_LearningPaths(): 
    ...
    ph = [0, 4*np.pi]
    ff = [0, 0]
    param_rng = {'x':x_rng, 'amp':amp_rng, 'freq':w, 'phase':ph, 'function':ff, "innovation_sd":n_sd }
    FG = FunGenerator(num_pts=Ntrain, batch_size=batch_size, param_range=param_rng, dim_input=1, dim_output=1)
    
    # Generate one set of random weight matrices and bias vectors ===================================
    W = GenerateInitializations(N_set=1, Hn=Hn, dim_input=1, dim_output=1, sigma=0.01, fnm=None)

    PARAM, LOSS_PATH, ITER = [], [], []

    for i in range(Ntrials):
        print('trial : ', i)
        param_final, param_start, loss_path, niter = LearningPath(InitParam=W[0], 
                                                                  FG=FG, \
                                                                  num_pts=Ntrain, \
                                                                  epochs=epochs, \
        ...
        ITER.append(niter)
    return PARAM, LOSS_PATH, ITER    

def LearningPath(InitParam, FG, num_pts, epochs, Loss_threshold, LR_init, Lambda=0.0):
    ''' 
    Fit a two-layer FC net to 1-dim data
    The initialization of net's parameters are given in InitParam.
    '''
    ...
    W1 = tf.get_variable('W1', initializer=tf.constant(InitParam['w1'], dtype=tf.float32))
    W2 = tf.get_variable('W2', initializer=tf.constant(InitParam['w2'], dtype=tf.float32))
    W3 = tf.get_variable('W3', initializer=tf.constant(InitParam['w3'], dtype=tf.float32))
    b1 = tf.get_variable('b1', initializer=tf.constant(InitParam['b1'], dtype=tf.float32))
    b2 = tf.get_variable('b2', initializer=tf.constant(InitParam['b2'], dtype=tf.float32))
    b3 = tf.get_variable('b3', initializer=tf.constant(InitParam['b3'], dtype=tf.float32))
    ...
    # choices of optimizers : use Adam, but does it really matter?
    ...
    while True:
        ...
        sess.run(train_step, feed_dict={xs:x_train[0,:,:], ys:y_train[0,:,:], lr:learning_sch})
        lss = sess.run(loss, feed_dict={xs:x_train[0,:,:], ys:y_train[0,:,:], lr:learning_sch})
        loss_path.append(lss)

        # make learning rate a function of trailing losses
        ave = np.mean(loss_path)
        ma = np.mean(loss_path[-100:])
        std_ma = np.std(loss_path[-100:])
        
        # SCHEDULE : lower learning rate if loss gets smaller, otherwise increase it
        # if i > 1000 and (i - i_last) > 2000:
        #     if lss < ma:
        ...
        if i % 100 == 0:
            print('Iter : %d, loss : %3.5f, ma : %3.5f' %(i, lss, ma))
        
        if i > epochs:
            print('Final ave loss : ', ma)
            break
        i = i + 1
...
# ============================================================================
# Args
#   InitParam : starting values of the neural net parameters, list of arrays
#   Ntrain, Nval : number of data points for training and validation 
#   loss_threshold : threshold below which training will end
#   max_iter, min_iter : maximum and minimum numbers of iterations
#   Lambda : regularization constant
#
# Return :
#    param : matrix where rows corresponds to the M functional shapes, columns the flattened Weight and Bias parameters
#    dist_from_start : list of length M, each entry is the distance of the final parameter vector from its starting values 
#    loss_train : list of length M, training losses at convergence/termination
#    loss_val : list of length M, validation losses at convergence/termination
def TrainAllFun(InitParam, Ntrain, Nval, loss_threshold, max_iter, Lambda, gnm):
    # NOTE : loss_train_cycle is a list of lists
    ...
    for j in range(N_fun):
        ...
        reg_terms = Lambda*(tf.nn.l2_loss(W1) + tf.nn.l2_loss(W2) + tf.nn.l2_loss(W3))
        loss = loss0 + reg_terms
        # could use any optimizer, e.g., Adam, ... doesn't really matter
        train_step = tf.train.RMSPropOptimizer(0.005, 0.9).minimize(loss)
...
def run_TrainAllFun(): 
    ...
    initW1, initW2, initW3, initb1, initb2, initb3 = GenerateInitializations(N_init, Hn, sigma=0.05, fnm=None)
...
def run_MultipleGradPaths(): 
    ...
    initW1, initW2, initW3, initb1, initb2, initb3 = GenerateInitializations(N_set=Ntrials, Hn=Hn, dim_input=1, dim_output=1, sigma=0.1, fnm=None)
...
def run_PerturbGradPaths(): 
    ...
    initW1, initW2, initW3, initb1, initb2, initb3 = GenerateInitializations(N_set=1, Hn=Hn, dim_input=1, dim_output=1, sigma=0.1, fnm=None)
    iParam = [initW1[0], initW2[0], initW3[0], initb1[0], initb2[0], initb3[0]]
